/**
 * CV-VAL-040: accuracy table — none (cv_val034) vs structured agents (cv_val040).
 * Usage: npx tsx src/scripts/cv-val-040-eval.ts
 */
...
const MODES = ["none", "structured_agents"] as const;
...
function labelFor(mode: (typeof MODES)[number], n: number): string {
  ...
  return mode === "structured_agents"
    ? `cv_val040_structured_agents_${tail}`
...
async function main(): Promise<void> {
  ...
  const runId = process.env.CV_VAL040_RUN_ID?.trim() || RUN_ID_DEFAULT;
