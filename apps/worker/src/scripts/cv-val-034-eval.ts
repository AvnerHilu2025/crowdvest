/**
 * CV-VAL-034: compare none vs minimal_memory vs current (cv_val034 labels).
 * Usage: npx tsx src/scripts/cv-val-034-eval.ts
 */
...
const MODES = ["none", "minimal_memory", "current"] as const;
...
function labelFor(mode: (typeof MODES)[number], n: number): string {
  return `cv_val034_${mode}_syn${numTok(SYN)}_info${numTok(INFO)}_evt${numTok(EVT)}_reg${numTok(REG)}_th${numTok(TH)}_ds${numTok(DS)}_n${n}`;
}
...
async function main(): Promise<void> {
  ...
  const runId = process.env.CV_VAL034_RUN_ID?.trim() || RUN_ID_DEFAULT;
