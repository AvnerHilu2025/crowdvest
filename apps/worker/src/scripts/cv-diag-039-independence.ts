/**
 * CV-DIAG-039: independence-style proxies (none vs cv_val039 diverse_tuned @ n=10000).
 * Usage: npx tsx src/scripts/cv-diag-039-independence.ts [--runId <uuid>] [--assetSymbol SPY]
 */
...
const LABEL_039 = "cv_val039_diverse_tuned_syn0p4_info0p4_evt0p2_reg0p2_th0p02_ds0p7_n10000";
...
function parseArgv(): { runId: string; assetSymbol: string } {
  ...
  let runId = process.env.CV_DIAG039_RUN_ID?.trim() || RUN_ID_DEFAULT;
  let assetSymbol = "SPY";
...
async function main(): Promise<void> {
  ...
  console.log("=== CV-DIAG-039 Independence proxies (read-only) ===\n");
  console.log(
    [
      "AgentDecision.distortedSignal per agent per step.",
      `Labels: ${LABEL_NONE} vs ${LABEL_039}`,
  ...
  const tuned = await load(LABEL_039, "diverse_tuned");
  await prisma.$disconnect();

  if (!baseline) console.log("\n**none**: variant not found.\n");
  else printModeSection(baseline);
  if (!tuned) console.log("\n**diverse_tuned**: variant not found.\n");
  else printModeSection(tuned);
  ...
  if (baseline && tuned) {
    ...
    const d = cmp(baseline.corrProxy, tuned.corrProxy);
    const loCorr =
      !Number.isFinite(d) ? "n/a" : d < 0 ? "none" : d > 0 ? "diverse_tuned" : "tie";
    const de = cmp(baseline.meanEntropy, tuned.meanEntropy);
    const hiEnt =
      !Number.isFinite(de) ? "n/a" : de > 0 ? "none" : de < 0 ? "diverse_tuned" : "tie";
    const dv = cmp(baseline.meanVarWithin, tuned.meanVarWithin);
    const wide =
      !Number.isFinite(dv)
        ? "n/a"
        : dv > 0
