/**
 * CV-VAL-046: mean_signal_sign accuracy table for mix sweep (read-only).
 * Usage: npx tsx src/scripts/cv-val-046-eval.ts [--runId <uuid>] [--assetSymbol SPY]
 */
...
function labelFor(mixId: number, n: number): string {
  return `cv_val046_mix${mixId}_n${n}`;
}
...
function parseArgv(): { runId: string; assetSymbol: string } {
  ...
  let runId = process.env.CV_VAL046_RUN_ID?.trim() || RUN_ID_DEFAULT;
  let assetSymbol = "SPY";
