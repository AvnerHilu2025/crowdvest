/**
 * CV-VAL-040: structured agents (--cvVal040 + --transformMode none → cv_val040_structured_agents_*).
 * Logic: decide.ts CV-ARCH-040 (trend/contrarian/noise/fundamental).
 * Usage:
 *   npx tsx src/scripts/cv-val-040-run.ts --runId <uuid> [--assetSymbol SPY] [--seed 0] [--steps 20]
 */
...
async function main(): Promise<void> {
  ...
  console.log(
    `[CV-VAL-040] runId=${runId} assetSymbol=${assetSymbol} seed=${seed} steps=${steps} agentNs=${AGENT_NS.join(",")} (transformMode=none + --cvVal040)`,
  );
  ...
  for (const n of AGENT_NS) {
    ...
    const label = `cv_val040_structured_agents_${labelTail}`;
    runStep(
