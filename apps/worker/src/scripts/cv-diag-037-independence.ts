/**
 * CV-DIAG-037: independence-style proxies (current vs none @ n10000).
 * Usage: npx tsx src/scripts/cv-diag-037-independence.ts [--runId <uuid>] [--assetSymbol SPY]
 */
...
const LABEL_CURRENT = "cv_val034_current_syn0p4_info0p4_evt0p2_reg0p2_th0p02_ds0p7_n10000";
...
function parseArgv(): { runId: string; assetSymbol: string } {
  ...
  let runId = process.env.CV_DIAG037_RUN_ID?.trim() || RUN_ID_DEFAULT;
  let assetSymbol = "SPY";
...
type ModeStats = {
  ...
  label: string;
  steps: StepStats[];
  /** Var_step(mean_i signal) / mean_step(var_i signal). */
  corrProxy: number;
...
async function main(): Promise<void> {
  ...
  console.log("=== CV-DIAG-037 Independence proxies (read-only) ===\n");
  console.log(
    [
      "AgentDecision.distortedSignal per agent per step.",
      `Labels: ${LABEL_CURRENT} vs ${LABEL_NONE}`,
  ...
  const cur = await load(LABEL_CURRENT, "current");
  const none = await load(LABEL_NONE, "none");
  await prisma.$disconnect();

  if (!cur) console.log("\n**current**: variant not found.\n");
  else printModeSection(cur);
  if (!none) console.log("\n**none**: variant not found.\n");
  else printModeSection(none);
  ...
  if (cur && none) {
    ...
    const d = cmp(cur.corrProxy, none.corrProxy);
    const loCorr =
      !Number.isFinite(d) ? "n/a" : d < 0 ? "current" : d > 0 ? "none" : "tie";
    const de = cmp(cur.meanEntropy, none.meanEntropy);
    const hiEnt =
      !Number.isFinite(de) ? "n/a" : de > 0 ? "current" : de < 0 ? "none" : "tie";
    const dv = cmp(cur.meanVarWithin, none.meanVarWithin);
    const wide =
      !Number.isFinite(dv)
        ? "n/a"
        : dv > 0
