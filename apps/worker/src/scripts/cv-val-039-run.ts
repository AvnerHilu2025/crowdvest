/**
 * CV-VAL-039: tuned diversity (--cvVal039 + --transformMode none → cv_val039_diverse_tuned_*).
 * Ranges in decide.ts: noiseSigma 0–0.02, sensitivity/thresholdFactor 0.9–1.1.
 * Usage:
 *   npx tsx src/scripts/cv-val-039-run.ts --runId <uuid> [--assetSymbol SPY] [--seed 0] [--steps 20]
 */
...
async function main(): Promise<void> {
  ...
  console.log(
    `[CV-VAL-039] runId=${runId} assetSymbol=${assetSymbol} seed=${seed} steps=${steps} agentNs=${AGENT_NS.join(",")} (transformMode=none + --cvVal039)`,
  );
  ...
  for (const n of AGENT_NS) {
    ...
    const label = `cv_val039_diverse_tuned_${labelTail}`;
    runStep(
