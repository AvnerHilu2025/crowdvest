/**
 * CV-DIAG-038: independence-style proxies (none vs cv_val038 diverse_agents @ n=10000).
 * Usage: npx tsx src/scripts/cv-diag-038-independence.ts [--runId <uuid>] [--assetSymbol SPY]
 */
...
const LABEL_038 = "cv_val038_diverse_agents_syn0p4_info0p4_evt0p2_reg0p2_th0p02_ds0p7_n10000";
...
function parseArgv(): { runId: string; assetSymbol: string } {
  ...
  let runId = process.env.CV_DIAG038_RUN_ID?.trim() || RUN_ID_DEFAULT;
  let assetSymbol = "SPY";
...
async function main(): Promise<void> {
  ...
  console.log("=== CV-DIAG-038 Independence proxies (read-only) ===\n");
  console.log(
    [
      "AgentDecision.distortedSignal per agent per step.",
      `Labels: ${LABEL_NONE} vs ${LABEL_038}`,
  ...
  const diverse = await load(LABEL_038, "diverse_agents");
  await prisma.$disconnect();

  if (!baseline) console.log("\n**none**: variant not found.\n");
  else printModeSection(baseline);
  if (!diverse) console.log("\n**diverse_agents**: variant not found.\n");
  else printModeSection(diverse);
  ...
  if (baseline && diverse) {
    ...
    const d = cmp(baseline.corrProxy, diverse.corrProxy);
    const loCorr =
      !Number.isFinite(d) ? "n/a" : d < 0 ? "none" : d > 0 ? "diverse_agents" : "tie";
    const de = cmp(baseline.meanEntropy, diverse.meanEntropy);
    const hiEnt =
      !Number.isFinite(de) ? "n/a" : de > 0 ? "none" : de < 0 ? "diverse_agents" : "tie";
    const dv = cmp(baseline.meanVarWithin, diverse.meanVarWithin);
    const wide =
      !Number.isFinite(dv)
        ? "n/a"
        : dv > 0
