/**
 * CV-VAL-025: plurality accuracy for gold-config variants.
 * Usage: npx tsx src/scripts/cv-val-025-eval.ts
 */
...
async function main(): Promise<void> {
  ...
  const header = `| agents | accuracy |`;
  const sep = `|--------|----------|`;
  const lines = [header, sep];
  for (const n of NS) {
    const label = `cv_val025_${n}`;
