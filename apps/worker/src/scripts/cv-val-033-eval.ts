/**
 * CV-VAL-033: compare transform modes at fixed ARCH-031-style weights + CV-VAL-029 decision rule.
 * Usage: npx tsx src/scripts/cv-val-033-eval.ts
 */
...
const MODES = ["current", "none", "smooth_only"] as const;
...
function labelFor(mode: (typeof MODES)[number], n: number): string {
  return `cv_val033_${mode}_syn${numTok(SYN)}_info${numTok(INFO)}_evt${numTok(EVT)}_reg${numTok(REG)}_th${numTok(TH)}_ds${numTok(DS)}_n${n}`;
}
...
async function main(): Promise<void> {
  ...
  const runId = process.env.CV_VAL033_RUN_ID?.trim() || RUN_ID_DEFAULT;
