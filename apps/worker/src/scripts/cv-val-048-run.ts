/**
 * CV-VAL-048: lean archetype mix sweep (--cvVal048MixLetter A–D, transformMode none → cv_val048_mix<Letter>_n<N>).
 * Usage:
 *   npx tsx src/scripts/cv-val-048-run.ts --runId <uuid> [--assetSymbol SPY] [--seed 0] [--steps 20]
 */
...
async function main(): Promise<void> {
  ...
  console.log(
    `[CV-VAL-048] runId=${runId} assetSymbol=${assetSymbol} seed=${seed} steps=${steps} mixLetters=${MIX_LETTERS.join(",")} agentNs=${AGENT_NS.join(",")}`,
  );
  ...
  for (const letter of MIX_LETTERS) {
    for (const n of AGENT_NS) {
      ...
      const label = `cv_val048_mix${letter}_n${n}`;
      runStep(
