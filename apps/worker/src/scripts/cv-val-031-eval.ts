/**
 * CV-VAL-031: baseline crowd-size accuracy (ARCH-031 preset weights).
 * Usage: npx tsx src/scripts/cv-val-031-eval.ts
 */
...
async function main(): Promise<void> {
  ...
  const runId = process.env.CV_VAL031_RUN_ID?.trim() || RUN_ID_DEFAULT;
  ...
  for (const n of LABEL_NS) {
    const label = `cv_val031_baseline_${n}`;
