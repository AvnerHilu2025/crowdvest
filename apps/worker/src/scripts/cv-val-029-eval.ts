/**
 * CV-VAL-029: baseline crowd-size accuracy for ARCH-029 (threshold decision path).
 * Usage: npx tsx src/scripts/cv-val-029-eval.ts
 */
...
const LABEL_NS = [100, 2000, 10_000] as const;
...
async function main(): Promise<void> {
  ...
  const runId = process.env.CV_VAL029_RUN_ID?.trim() || RUN_ID_DEFAULT;
  ...
  const lines: string[] = [];
  lines.push("| agents | accuracy |");
  lines.push("|--------|----------|");
  for (const n of LABEL_NS) {
    const label = `cv_val029_baseline_${n}`;
