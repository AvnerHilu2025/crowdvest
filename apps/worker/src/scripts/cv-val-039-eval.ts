/**
 * CV-VAL-039: accuracy table — none (cv_val034) vs tuned diversity (cv_val039).
 * Usage: npx tsx src/scripts/cv-val-039-eval.ts
 */
...
const MODES = ["none", "diverse_tuned"] as const;
...
function labelFor(mode: (typeof MODES)[number], n: number): string {
  ...
  return mode === "diverse_tuned"
    ? `cv_val039_diverse_tuned_${tail}`
...
async function main(): Promise<void> {
  ...
  const runId = process.env.CV_VAL039_RUN_ID?.trim() || RUN_ID_DEFAULT;
